...
namespace Drupal\agnesian_custom\Plugin\views\filter;
 ...
 *
 * @ViewsFilter("entity_reference_target_id")
 */
class EntityReferenceTargetId extends ManyToOne {
  ...
  /**
   * Constructs an AgnesianCustomNodeId object.
   *
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    if (!empty($this->definition['node_type'])) {
      $this->options['node_type_id'] = $this->definition['node_type'];
    }
  }
  ...
  protected function defineOptions() {
    ...
    $options['node_type_id'] = array('default' => '');
  ...
  public function buildExtraOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    if ($this->options['limit']) {
      ...
      if (empty($this->options['node_type_id'])) {
        $first_type = reset($content_types);
        $this->options['node_type_id'] = $first_type->id();
      }

      if (empty($this->definition['node_type'])) {
        $form['node_type_id'] = array(
          '#type' => 'radios',
          '#title' => $this->t('Content type'),
          '#options' => $options,
          '#description' => $this->t('Select which content type to show nodes for in the regular options.'),
          '#default_value' => $this->options['node_type_id'],
        );
      }
    }
  ...
  protected function valueForm(&$form, FormStateInterface $form_state) {
    $content_type = $this->nodeTypeStorage->load($this->options['node_type_id']);
    if (empty($content_type) && $this->options['limit']) {
      $form['markup'] = array(
    ...
    if ($this->options['type'] == 'textfield') {
      ...
      $form['value'] = array(
        '#title' => $this->options['limit'] ? $this->t('Select nodes from content type @type', array('@type' => $content_type->label())) : $this->t('Select nodes'),
        '#type' => 'textfield',
      ...
      if ($this->options['limit']) {
        ...
        $form['value']['#selection_settings']['target_bundles'] = array($content_type->id());
  ...
  public function calculateDependencies() {
    ...
    $content_type = $this->nodeTypeStorage->load($this->options['node_type_id']);
    $dependencies[$content_type->getConfigDependencyKey()][] = $content_type->getConfigDependencyName();
